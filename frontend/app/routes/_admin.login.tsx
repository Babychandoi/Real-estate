      ...
      <form onSubmit={submit} className="w-full rounded-2xl border border-outline-variant/60 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 space-y-4">
          <label className="block text-sm font-semibold text-on-surface">Email
            <span className="relative mt-1.5 block"><Mail className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-on-surface-variant" /><input name="admin-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="section-admin username" required className="min-h-11 w-full rounded-lg border border-outline-variant bg-white py-2 pl-10 pr-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" placeholder="admin@nhadatchuan.online" /></span>
          </label>
          <label className="block text-sm font-semibold text-on-surface">Mật khẩu
            <span className="relative mt-1.5 block"><LockKeyhole className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-on-surface-variant" /><input name="admin-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="section-admin current-password" required className="min-h-11 w-full rounded-lg border border-outline-variant bg-white py-2 pl-10 pr-11 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" placeholder="Nhập mật khẩu" /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'} className="absolute right-1 top-0 grid h-11 w-11 place-items-center text-on-surface-variant hover:text-on-surface">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span>
          </label>
        </div>
