  ...
  return <main className="mx-auto max-w-3xl px-4 py-8 md:px-8">
    ...
    <form onSubmit={save} className="mt-6 space-y-6 rounded-2xl border border-outline-variant/50 bg-surface-container-lowest p-5 md:p-7">
      <section className="flex items-center gap-5 border-b border-outline-variant/40 pb-6">
        <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full bg-primary/10 text-lg font-bold text-primary">{avatarMediaUrl ? <img src={avatarMediaUrl} alt="Ảnh đại diện" className="h-full w-full object-cover" /> : initials || <UserRound className="h-7 w-7" />}</div>
        <div><label className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border border-outline-variant px-4 text-sm font-semibold text-on-surface hover:bg-surface-container"><Camera className="h-4 w-4" />{uploading ? 'Đang tải ảnh…' : 'Đổi ảnh đại diện'}<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={uploading} onChange={(event) => void uploadAvatar(event.target.files?.[0])} /></label><p className="mt-2 text-xs text-on-surface-variant">Ảnh này được hiển thị công khai cùng tin đăng của bạn.</p></div>
      </section>
