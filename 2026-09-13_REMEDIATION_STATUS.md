...
- SEO kỹ thuật: URL công khai của tin chỉ dùng slug tiêu đề lưu trong PostgreSQL, không lộ UUID; unique constraint và hậu tố số xử lý tiêu đề trùng. URL UUID cũ vẫn được chuẩn hóa về canonical; trang chi tiết cập nhật title, description, Open Graph, Twitter Card và JSON-LD từ dữ liệu API; `robots.txt` và sitemap động chỉ xuất các tin `ACTIVE` đã được kiểm tra HTTP 200 trên domain.
